...
<jstl:if test="${test}">
	<jstl:choose>
		...
		<jstl:when test="${newTab == true}">		
			<jstl:set var="script" value="pushCurrentUrl(); redirect('${action}', '_blank'))"/>			
		</jstl:when>
		<jstl:otherwise>
			<jstl:set var="script" value="pushCurrentUrl(); redirect('${action}')"/>
		</jstl:otherwise>		
	</jstl:choose>	
	<a onclick="javascript: ${script}" style="text-decoration: underline; cursor: pointer">
